...
test-unit: install
	...
	poetry run pytest tests/unit $(PYTEST_OPTIONS)
...
.PHONY: test-int
test-int: install
	@ if test -e $(FAILURES); then poetry run pytest tests/int $(PYTEST_RERUN_OPTIONS); fi
	@ rm -rf $(FAILURES)
	poetry run pytest tests/int $(PYTEST_OPTIONS)
ifndef DISABLE_COVERAGE
